...
/*
 * A string block consists of a null-terminated block of null-terminated
 * strings. Note that an ANSI environment block is terminated by two zero bytes:
 * one for the last string, one more to terminate the block. A Unicode
 * environment block is terminated by four zero bytes: two for the last string,
